...
describe("crypto", () => {
    ...
    it("fails with wrong AAD", () => {
        ...
        const plaintext = Buffer.from("hello");
        const encrypted = encryptAesGcm(plaintext, aad, dek);
        expect(() => decryptAesGcm(encrypted.ciphertext, wrongAad, dek, encrypted.nonce, encrypted.tag)).toThrow();
    });
    it("rejects invalid nonce length", () => {
        ...
        expect(() => decryptAesGcm(encrypted.ciphertext, aad, dek, badNonce, encrypted.tag)).toThrow(/Invalid nonce length/);
    });
