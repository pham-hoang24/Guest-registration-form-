...
describe("worker idempotency", () => {
    beforeEach(() => {
        if (!(db instanceof InMemoryDb))
            throw new Error("worker tests require InMemoryDb — unset SQL_SERVER");
        if (!(storage instanceof InMemoryBlobStore))
            throw new Error("worker tests require InMemoryBlobStore — unset BLOB_ACCOUNT_URL");
        db.reset();
        storage.reset();
...
describe("worker DB-backed payload", () => {
    ...
    it("loads and decrypts payload from store when job has no payload", async () => {
        ...
        const encrypted = await encryptPayload(JSON.stringify({ name: "Bob" }), {
