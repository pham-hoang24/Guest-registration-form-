...
beforeAll(async () => {
    if (!(db instanceof InMemoryDb))
        throw new Error("Requires InMemoryDb — unset SQL_SERVER");
    if (!(storage instanceof InMemoryBlobStore))
        throw new Error("Requires InMemoryBlobStore — unset BLOB_ACCOUNT_URL");
...
function makeToken(opts) {
    return jwt.sign({ tenantId: opts.tenantId, propertyIds: opts.propertyIds }, DEV_SECRET, { algorithm: "HS256", subject: opts.userId, expiresIn: "1h" });
}
async function setupReadySubmission(tenantId, propertyId) {
    const inDb = db;
...
describe("GET /v1/owner/submissions/:id/pdf", () => {
    ...
    it("returns 500 when ciphertext has been tampered (hash mismatch)", async () => {
        ...
        await storage.put(record.blobPath, Buffer.from("tampered-garbage-bytes"), "application/octet-stream");
        const token = makeToken({ userId: "owner-1", tenantId: "tenant-1", propertyIds: ["prop-1"] });
        const res = await fetch(`${baseUrl}/v1/owner/submissions/${submission.id}/pdf`, {
            headers: { Authorization: `Bearer ${token}` }
        });
