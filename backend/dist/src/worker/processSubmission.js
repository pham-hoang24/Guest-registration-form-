...
import { ENCRYPTED_PDF_RECORD_VERSION } from "../storage/encryptedPdfRecord.js";
...
export const setKekAdapterForTests = (adapter) => {
    kekAdapter = adapter;
};
export const processSubmissionJob = async (job) => {
    ...
    if (job.payload !== undefined) {
        ...
        try {
            const plaintext = await decryptPayload(encryptedRecord, {
                tenantId: submission.tenantId,
                propertyId: submission.propertyId,
                submissionId: submission.id
            }, getKekAdapter());
...
export const processSubmissionToEncryptedPdf = async (submission, payload, templateRef) => {
    const pdfResult = await generatePdf(payload, templateRef.templateId, templateRef.templateVersion);
    const dek = generateDek();
    const aadBytes = buildAadBytes({
        tenantId: submission.tenantId,
...
const verifyExistingRecord = async (record) => {
