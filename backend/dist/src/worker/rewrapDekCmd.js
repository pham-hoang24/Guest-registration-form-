...
// ---------------------------------------------------------------------------
const missing = [];
if (!values["old-key-id"])
    missing.push("--old-key-id");
if (!values["new-key-id"])
    missing.push("--new-key-id");
if (missing.length > 0) {
    console.error(`[rewrapDekCmd] Missing required arguments: ${missing.join(", ")}`);
    console.error("Usage: node dist/worker/rewrapDekCmd.js " +
 ...
 */
const assertSafeKeyId = (value, label) => {
    ...
    if (!value.startsWith("https://") || !value.includes(".vault.azure.net/")) {
        console.error(`[rewrapDekCmd] ${label} does not look like a valid Azure Key Vault key URI ` +
...
if (values["old-key-version"])
    assertSafeKeyId(values["old-key-version"], "--old-key-version");
if (values["new-key-version"])
    assertSafeKeyId(values["new-key-version"], "--new-key-version");
...
if (oldKeyId === newKeyId && oldKeyVersion === newKeyVersion) {
    console.error("[rewrapDekCmd] --old-key-id/version and --new-key-id/version are identical. " +
...
try {
    ...
    console.error("[rewrapDekCmd] Failed to initialise Key Vault adapter:", err instanceof Error ? err.message : err);
    process.exit(1);
}
const input = {
...
if (isDryRun) {
    ...
    try {
        ...
        let offset = 0;
        while (true) {
            const page = await db.getEncryptedPdfRecordsByKek(input.oldKekKeyId, input.oldKekKeyVersion, { offset, limit: PAGE_SIZE });
            if (page.length === 0)
                break;
            count += page.length;
            offset += PAGE_SIZE;
        ...
        console.error("[rewrapDekCmd] DRY RUN failed — could not query database:", err instanceof Error ? err.message : err);
        process.exit(1);
    }
    process.exit(0);
}
...
try {
    ...
    if (result.totalFailed > 0) {
        console.error(`[rewrapDekCmd] ${result.totalFailed} record(s) failed — inspect audit log for details. ` +
    ...
    console.error("[rewrapDekCmd] Fatal error during rewrap job:", err instanceof Error ? err.message : err);
    process.exit(1);
}
