 ...
 */
export const encryptPayload = async (plaintext, context, kekAdapter) => {
 ...
 */
export const decryptPayload = async (record, context, kekAdapter) => {
    const aadBytes = buildPayloadAadBytes({
    ...
    const dek = await kekAdapter.unwrapDek(record.wrappedDek, record.kekKeyId, record.kekKeyVersion);
    const plaintext = decryptAesGcm(record.ciphertext, aadBytes, dek, record.nonce, record.tag);
