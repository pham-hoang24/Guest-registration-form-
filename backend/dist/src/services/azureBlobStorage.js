...
export class AzureBlobStorage {
    ...
    constructor() {
        ...
        if (!accountUrl)
            throw new Error("BLOB_ACCOUNT_URL is required");
        if (!container)
            throw new Error("BLOB_CONTAINER is required");
    ...
    async put(path, data, contentType) {
        ...
        if (contentType !== "application/octet-stream") {
            throw new Error(`Blob content type must be "application/octet-stream" for ciphertext blobs, got "${contentType}"`);
        }
